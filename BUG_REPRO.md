...
--- FAIL: TestCreateProfileRejectsSameSensorWithDifferentCase (0.00s)
    server_test.go:47: second status = 201, want 409
FAIL
FAIL	buoy-calibration/internal/api	0.005s
...
同一种传感器即使使用不同大小写也只能存在一份标定档案，避免运行选用不一致的校正参数。
